...
public class PacketOpenMindControlMenu implements IMessage {
    ...
    UUID target; int entityId; String name,superpowerId,superpowerName; boolean playerInventory,abilityAutocast; List<ItemStack> items=new ArrayList<>(); List<AbilityInfo> abilities=new ArrayList<>(); float health,maxHealth; int armor,remainingTicks;
    ...
    public static class Handler implements IMessageHandler<PacketOpenMindControlMenu,IMessage>{@Override public IMessage onMessage(PacketOpenMindControlMenu m,MessageContext c){HeroSMP.proxy.handleClientPacket(m);return null;}}
}
