...
/**
 * Time Stone ability: Slow Down Time.
 * Each activation reduces the tickrate in the player's current dimension by 5
 * (minimum 1.0). Only players inside the same dimension are affected; the
 ...
 */
public class AbilitySlowTime extends AbilityAction {

    private static final ResourceLocation ICON =
            new ResourceLocation("herosmp", "textures/abilities/pixel_rewind.png");

    private static final float STEP     = 5.0f;
    private static final float MIN_RATE = 1.0f;

    public AbilitySlowTime(EntityLivingBase entity) {
        super(entity);
    }
    ...
    public boolean action() {
        ...
        float next = Math.max(MIN_RATE, current - STEP);
